import Link from "next/link";
import StatusBadge from "./StatusBadge";

export interface Lead {
  id: number;
  ig_username: string;
  full_name: string | null;
  status: string;
  notes: string | null;
  created_at: string;
  updated_at: string;
}

interface LeadCardProps {
  lead: Lead;
  isDragging?: boolean;
}

export default function LeadCard({ lead, isDragging }: LeadCardProps) {
  const timeAgo = getTimeAgo(lead.updated_at);

  return (
    <Link
      href={`/leads/${lead.id}`}
      className="block animate-slide-in"
      style={{
        opacity: isDragging ? 0.6 : 1,
        transform: isDragging ? "rotate(2deg) scale(1.02)" : "none",
        transition: "all 0.15s ease",
      }}
    >
      <div className="card p-3.5 cursor-grab active:cursor-grabbing">
        <div className="flex items-start justify-between gap-2 mb-2.5">
          <div className="flex items-center gap-2.5 min-w-0">
            {/* Avatar */}
            <div
              className="w-8 h-8 rounded-full flex items-center justify-center text-[0.75rem] font-semibold flex-shrink-0"
              style={{
                background: "var(--accent-light)",
                color: "var(--accent)",
              }}
            >
              {lead.ig_username.charAt(0).toUpperCase()}
            </div>
            <div className="min-w-0">
              <p className="text-[0.8125rem] font-semibold truncate" style={{ color: "var(--foreground)" }}>
                @{lead.ig_username}
              </p>
              {lead.full_name && (
                <p className="text-xs truncate" style={{ color: "var(--muted)" }}>
                  {lead.full_name}
                </p>
              )}
            </div>
          </div>
        </div>

        {lead.notes && (
          <p
            className="text-xs line-clamp-2 mb-2.5 leading-relaxed"
            style={{ color: "var(--foreground-secondary)" }}
          >
            {lead.notes}
          </p>
        )}

        <div className="flex items-center justify-between">
          <StatusBadge status={lead.status} />
          <span className="text-[0.6875rem]" style={{ color: "var(--muted)" }}>
            {timeAgo}
          </span>
        </div>
      </div>
    </Link>
  );
}

function getTimeAgo(dateStr: string): string {
  const now = new Date();
  const date = new Date(dateStr + "Z");
  const diffMs = now.getTime() - date.getTime();
  const diffMins = Math.floor(diffMs / 60000);
  if (diffMins < 1) return "just now";
  if (diffMins < 60) return `${diffMins}m ago`;
  const diffHours = Math.floor(diffMins / 60);
  if (diffHours < 24) return `${diffHours}h ago`;
  const diffDays = Math.floor(diffHours / 24);
  if (diffDays < 30) return `${diffDays}d ago`;
  return date.toLocaleDateString();
}
